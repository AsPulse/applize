...
export function ClientInitialize(applizeRoot: string) {
  ...
  if ( content ) {
    ...
    window.__applize.pageMove = (pathname: string, targetElement: HTMLElement) => {
        ...
        xhr.addEventListener('load', () => {
          ...
          if(window.__applize) window.__applize.render = new DOMRendererClient<Record<never, never>>(
            fragment,
            applizeRoot,
            () => {
