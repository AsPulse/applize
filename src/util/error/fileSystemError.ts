...
const isErrorSerializable = (errorObject: unknown): errorObject is ISerializableError => {
...
export class FileSystemError {
  ...
  constructor(code: string, errno?: number,  syscall?: string, path?: string) {
    ...
    this.path = path;
    this.code = errorTypes.find(v => v.code === code) ?? { code: code, type: 'Unknown' };
  }
}
export function FileSystemErrorSerialize(errorObject: unknown): FileSystemError {
  if ( isErrorSerializable(errorObject) ) {
    return new FileSystemError(errorObject.code, errorObject.errno, errorObject.syscall, errorObject.path);
  } else {
