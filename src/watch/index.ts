...
export function applizeWatch(
  ...
  new Watcher(directory, builder, target, () => new Promise(resolve => {
...
export class Watcher {
  ...
  constructor(public directory: string, builder: string, public target: string, public refresher: () => Promise<boolean>, public reExecutor: () => Promise<void>) {
    watch(directory, { ignored: [builder, dirname(target), /node_modules/, /\.git/] }).on('all', () => {
...
export async function getSnapShot(directory: string, target: string): Promise<Snapshot[]> {
  ...
  const hashes = (await getAllFilesInDir(directory, []))
    ...
    .map(v =>  new Promise<Snapshot>(promiseResolve => {
...
export function equalSnapshot(a: Snapshot[], b: Snapshot[]) {
  ...
  if(keys.filter(v => !aKeys.includes(v)).length > 0 || keys.filter(v => !bKeys.includes(v)).length > 0) {
    return false;
  }
  return (keys.every(v => a.find(e => e.path === v)?.digest === b.find(e => e.path === v)?.digest));
}
